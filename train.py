...
class GPT(nn.Module):
    ...
    def _compute_window_sizes(self, config):
        ...
        short_window = int(long_window * 0.125)
        ...
        for path, param in flat_params:
            if use_muon and "blocks" in path and param.ndim == 2:
                ...
                self.param_config[path] = {
                    ...
                    "betas": adam_betas, "eps": 1e-10, "weight_decay": 0,
                }
                ...
                self.param_config[path] = {
                    ...
                    "betas": adam_betas, "eps": 1e-10, "weight_decay": 0.0025,
                }
...
# Optimization (leaderboard baseline: M4 Max val_bpb=1.294526)
USE_MUON = True
NS_STEPS = 5
MUON_MOMENTUM = 0.85
MUON_BETA2 = 0.97425
...
while True:
    ...
    if train_loss_f > 100 or train_loss_f != train_loss_f:
        # NaN or diverged — exit immediately to free the slot
        peak_vram_mb = get_peak_memory_mb()
        print(f"\nFAIL: loss={'nan' if train_loss_f != train_loss_f else f'{train_loss_f:.1f}'} at step {step}")
        print("---")
        print("val_bpb:          nan")
